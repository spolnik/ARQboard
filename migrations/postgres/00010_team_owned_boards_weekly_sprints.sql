-- +goose Up
CREATE TEMP TABLE board_team_reassignments AS
SELECT
    id AS board_id,
...
SELECT
    ...
    boards.workspace_id,
    boards.name,
    boards.slug || '-team-' || substr(board_team_reassignments.new_team_id::text, 1, 8)
...
UPDATE sprints
SET team_id = board_team_reassignments.new_team_id
FROM board_team_reassignments
...
CREATE TEMP TABLE team_board_backfills AS
SELECT
    teams.id AS team_id,
...
SELECT
    ...
    teams.id,
    teams.name,
    teams.slug || '-board-' || substr(team_board_backfills.board_id::text, 1, 8),
    'Team-owned board.'
...
SELECT gen_random_uuid(), board_id, 'Planned', 'planned', 0
FROM team_board_backfills
UNION ALL
SELECT gen_random_uuid(), board_id, 'In progress', 'in_progress', 1
FROM team_board_backfills
UNION ALL
SELECT gen_random_uuid(), board_id, 'Ready for review', 'ready_for_review', 2
FROM team_board_backfills
UNION ALL
SELECT gen_random_uuid(), board_id, 'Done', 'done', 3
FROM team_board_backfills;
